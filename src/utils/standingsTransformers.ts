...
import { FormattedStandings, GroupingsMode, LineChartDataset, StandingsForBoxscore, StandingsMode, TeamRecord } from "../types/standings";
...
export const transformStandingsForBoxscore = (
    ...
    const standingsForBoxscore: StandingsForBoxscore = [awayTeamWinsLosses, homeTeamWinsLosses];
    ...
    const allProcessedRecords = await Promise.all(
        standingsJson.records.map(async (record: any) => {
            const updatedTeamRecords = await Promise.all(
                (record.teamRecords || []).map(async (teamRec: any): Promise<TeamRecord> => {
                    const teamDetails = await fetchTeamDetails(teamRec.team.id);
                    const teamName = teamDetails?.teams?.[0]?.name || "Unknown Team";
    ...
    switch (standingsMode) {
        case 'regular season':
            switch (groupingsMode) {
                case 'division':
                    return Promise.all(
                        allProcessedRecords.map(async (record: any): Promise<FormattedStandings> => {
                            const divisionData = await fetchDivision(record.division.id);
                            return {
                                division: divisionData?.divisions?.[0]?.nameShort || "Unknown",
                ...
                case 'league': {
                    const americanLeagueTeams = allProcessedRecords
                        ...
                        .toSorted((a, b) => parseInt(a.leagueRank || '0') - parseInt(b.leagueRank || '0'));
                    ...
                    return [
                        { division: 'American League', teamRecords: americanLeagueTeams },
                        { division: 'National League', teamRecords: nationalLeagueTeams },
                    ];
                }
                ...
                case 'MLB': {
                    const mlbTeams = allProcessedRecords
                        .flatMap(r => r.teamRecords)
                        .toSorted((a, b) => parseInt(a.sportRank || '0') - parseInt(b.sportRank || '0'));

                    return [{ division: 'Major League Baseball', teamRecords: mlbTeams }];
                }
        ...
        case 'spring training': {
            const grapefruitLeagueTeams = allProcessedRecords
                .map(division => ({
                    ...division,
                    teamRecords: division.teamRecords.filter(r => r.team?.springLeague?.id === 115)
                }))
                .filter(division => division.teamRecords.length > 0)
                .flatMap(r => r.teamRecords)
                .toSorted((a, b) => parseInt(a.springLeagueRank || '0') - parseInt(b.springLeagueRank || '0'));

            const cactusLeagueTeams = allProcessedRecords
                .map(division => ({
                    ...division,
                    teamRecords: division.teamRecords.filter(r => r.team?.springLeague?.id === 114)
                }))
                .filter(division => division.teamRecords.length > 0)
                .flatMap(r => r.teamRecords)
                .toSorted((a, b) => parseInt(a.springLeagueRank || '0') - parseInt(b.springLeagueRank || '0'));
            ...
            return [
                { division: 'Grapefruit League', teamRecords: grapefruitLeagueTeams },
...
export const transformLineChartStandings = (
    ...
    const teamRecordsByDate = scheduleJson.dates.map((date: any) => {
        (date.games || []).forEach((game: any) => {
            ...
            if (homeTeam && awayTeam && homeTeam in runningScores && awayTeam in runningScores) {
            ...
            return [
                {
                    ...
                    teamRecords: teamRecordsByDate.map(row => 
                        Object.fromEntries(
                            Object.entries(row).filter(([key]) => americanLeagueTeams.has(key))
                        )
                    )
                },
                {
                    ...
                    teamRecords: teamRecordsByDate.map(row => 
                        Object.fromEntries(
                            Object.entries(row).filter(([key]) => nationalLeagueTeams.has(key))
                        )
                    )
                }
            ];
