...
export type TeamName = keyof typeof Consts.teamInfo;
...
class Consts {
    ...
    static findTeamIndex(teamName) {
        ...
        for (const league in Consts.teams) {
            for (
                let divisionIndex = 0;
                divisionIndex < Consts.teams[league].length;
                divisionIndex++
            ) {
    ...
    static dataTableContainerSx: SxProps<Theme> = {
        ...
        '& table.dataTable tbody tr.selected *, & table.dataTable tbody tr td.selected *':
            {
                backgroundColor: (theme) =>
                    `${theme.palette.custom.dark} !important`,
