...
function Play({ play, theme }) {
    ...
    const nextEnds = { ...ends };
    const mapBaseToKey = (key) =>
        ({ '1B': 'first', '2B': 'second', '3B': 'third' })[key] || key;
    ...
    return (
        <Accordion variant="outlined">
            <AccordionSummary
                expandIcon={<KeyboardDoubleArrowDown />}
                sx={{
                    backgroundColor: play?.about?.isScoringPlay
                        ? '#008800'
                        : theme.palette.custom.plays,
                    // borderBottom: '1px solid',
                    // borderColor: 'divider',
                    '&:hover': {
                        backgroundColor: play?.about?.isScoringPlay
                            ? '#009900'
                            : theme.palette.custom.playsHover,
                    },
                    // display: 'flex',
                    // "& .MuiAccordionSummary-content": {
                    //     justifyContent: "space-between",
            ...
            >
                ...
                >
                    ...
                    <Stack direction="row">
                        {Array.from({ length: 3 - play?.count?.outs }).map(
                            (_, i) => {
                                return (
                                    <CircleOutlined
                                        key={`empty-${i}`}
                        ...
                        {Array.from({ length: play?.count?.outs }).map(
                            (_, i) => {
                                return (
                                    <Circle
                                        key={`filled-${i}`}
            ...
            <AccordionDetails>
                <Typography>{play?.result?.description}</Typography>
                <Box
                    sx={{
                        display: 'grid',
...
function PlayIcon({ desc }) {
    ...
    if (desc !== undefined) {
        if (desc.includes('steals')) {
            ...
            desc.includes('Pickoff Attempt') ||
            desc.includes('picks off') ||
            desc.includes('Wild pitch') ||
            desc.includes('Passed ball') ||
            desc.includes('Delay')
        ) {
...
function StrikeZone({ pitches }) {
    ...
    const strikeZoneCanvasLeft =
        strikeZoneWidth / 2 - strikeZoneSide * sizeMult;
    const strikeZoneCanvasRight =
        strikeZoneWidth / 2 + strikeZoneSide * sizeMult;
    const strikeZoneCanvasTop =
        strikeZoneHeight / 2 - (strikeZoneVertSize / 2) * sizeMult;
    const strikeZoneCanvasBottom =
        strikeZoneHeight / 2 + (strikeZoneVertSize / 2) * sizeMult;
    ...
    return (
        <svg
            ...
            viewBox={`0 0 ${strikeZoneWidth} ${strikeZoneHeight}`}
            style={{ border: '1px solid #ffffff' }}
        >
            <line
                x1={strikeZoneCanvasLeft}
                y1={strikeZoneCanvasBottom}
                x2={strikeZoneCanvasLeft}
                y2={strikeZoneCanvasTop}
                stroke="#cccccc"
                strokeWidth="2"
            />
            <line
                x1={strikeZoneCanvasRight}
                y1={strikeZoneCanvasBottom}
                x2={strikeZoneCanvasRight}
                y2={strikeZoneCanvasTop}
                stroke="#cccccc"
                strokeWidth="2"
            />
            ...
            {pitches.map((pitch, i) => {
                ...
                const bottomY =
                    strikeZoneHeight / 2 + (strikeZoneVertSize / 2) * sizeMult;
                const percentFromBottomY =
                    (rawPitchY - strikeZoneBottom) / strikeZoneVertSize;
...
export const Plays = () => {
    ...
    useEffect(() => {
        const getPlays = async () => {
            ...
            setIsLoading(true);
            try {
                const inningsContent = await fetchGame(selectedGame, [
                    'credits',
                    'alignment',
                    'flags',
                ]);
